...
check_ci_workflow_checkout_wiring() {
  local workflow_file="$ROOT_DIR/.github/workflows/check.yml"
  local checkout_module_step=""
  local checkout_verus_step=""
  local checkout_module_line=""
  ...
  checkout_module_line="$(grep -nE '^[[:space:]]*- name:[[:space:]]*Checkout bootstrapped-llm-sim[[:space:]]*$' "$workflow_file" | head -n 1 | cut -d: -f1)"
  ...
  if [[ -z "$checkout_module_line" || -z "$checkout_verus_line" ]]; then
    echo "error: required checkout steps missing in $workflow_file"
    echo "expected steps: 'Checkout bootstrapped-llm-sim' and 'Checkout Verus'"
    exit 1
  fi
  ...
  if (( checkout_module_line >= build_line || checkout_module_line >= strict_line )); then
    echo "error: workflow checkout step order invalid for 'Checkout bootstrapped-llm-sim'"
  ...
  checkout_module_step="$(extract_named_workflow_step_block "$workflow_file" "Checkout bootstrapped-llm-sim")"
  checkout_verus_step="$(extract_named_workflow_step_block "$workflow_file" "Checkout Verus")"
  if [[ -z "$checkout_module_step" || -z "$checkout_verus_step" ]]; then
    echo "error: failed to parse required checkout step block(s) in $workflow_file"
    exit 1
  fi

  if ! printf '%s\n' "$checkout_module_step" | grep -Fq "uses: $CHECKOUT_ACTION_REF"; then
    echo "error: workflow 'Checkout bootstrapped-llm-sim' step must pin uses: $CHECKOUT_ACTION_REF"
    exit 1
  fi
  if ! printf '%s\n' "$checkout_module_step" | grep -qE 'path:[[:space:]]*bootstrapped-llm-sim'; then
    echo "error: workflow 'Checkout bootstrapped-llm-sim' step must set path: bootstrapped-llm-sim"
    exit 1
  fi
  if ! printf '%s\n' "$checkout_module_step" | grep -qE 'persist-credentials:[[:space:]]*false'; then
    echo "error: workflow 'Checkout bootstrapped-llm-sim' step must set persist-credentials: false"
    exit 1
  fi
...
check_ci_workflow_end_to_end_structure() {
  ...
  if ! printf '%s\n' "$strict_step" | grep -qE 'working-directory:[[:space:]]*bootstrapped-llm-sim'; then
    echo "error: workflow 'Run strict checks' step must run in bootstrapped-llm-sim"
    exit 1
  fi
...
run_cargo_verus_verify() {
  ...
  if command -v rustup >/dev/null 2>&1; then
    ...
    if [[ -n "$feature_flags" ]]; then
      ...
      "${CARGO_CMD[@]}" verus verify --manifest-path Cargo.toml -p bootstrapped-llm-sim $feature_flags -- --triggers-mode silent
    else
      "${CARGO_CMD[@]}" verus verify --manifest-path Cargo.toml -p bootstrapped-llm-sim -- --triggers-mode silent
    fi
    ...
    if nix-shell -p rustup --run "rustup --version >/dev/null 2>&1" >/dev/null 2>&1; then
      ...
      nix-shell -p rustup --run "
        ...
        export VERUS_Z3_PATH='$VERUS_SOURCE/z3'
        cd '$ROOT_DIR'
        cargo $OFFLINE_CARGO_FLAG verus verify --manifest-path Cargo.toml -p bootstrapped-llm-sim $feature_flags -- --triggers-mode silent
      "
    else
      skip_or_fail_verus_unavailable \
